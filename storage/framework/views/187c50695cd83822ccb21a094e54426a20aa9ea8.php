<section id="banner-wrapper">

    <div class="container">

        <!-- SECTION TITLE -->
        <div class="mb-7 text-center">

            <h6><?php echo e(__($frontend_sections->clients_title)); ?></h6>

        </div> <!-- END SECTION TITLE -->

        <div class="row" id="partners">
                    
            <div class="partner" data-aos="flip-down" data-aos-delay="100" data-aos-once="true" data-aos-duration="400">					
                <div class="partner-image d-flex">
                    <div>
                        <img src="<?php echo e(theme_url('img/frontend/logos/logo-1.svg')); ?>" alt="partner">
                    </div>
                </div>	
            </div>    
            
            <div class="partner" data-aos="flip-down" data-aos-delay="200" data-aos-once="true" data-aos-duration="400">					
                <div class="partner-image d-flex">
                    <div>
                        <img src="<?php echo e(theme_url('img/frontend/logos/logo-2.svg')); ?>" alt="partner">
                    </div>
                </div>	
            </div> 

            <div class="partner" data-aos="flip-down" data-aos-delay="300" data-aos-once="true" data-aos-duration="400">					
                <div class="partner-image d-flex">
                    <div>
                        <img src="<?php echo e(theme_url('img/frontend/logos/logo-3.svg')); ?>" alt="partner">
                    </div>
                </div>	
            </div> 

            <div class="partner" data-aos="flip-down" data-aos-delay="400" data-aos-once="true" data-aos-duration="400">					
                <div class="partner-image d-flex">
                    <div>
                        <img src="<?php echo e(theme_url('img/frontend/logos/logo-4.svg')); ?>" alt="partner">
                    </div>
                </div>	
            </div> 

            <div class="partner" data-aos="flip-down" data-aos-delay="500" data-aos-once="true" data-aos-duration="400">					
                <div class="partner-image d-flex">
                    <div>
                        <img src="<?php echo e(theme_url('img/frontend/logos/logo-5.svg')); ?>" alt="partner">
                    </div>
                </div>	
            </div> 

            <div class="partner" data-aos="flip-down" data-aos-delay="600" data-aos-once="true" data-aos-duration="400">					
                <div class="partner-image d-flex">
                    <div>
                        <img src="<?php echo e(theme_url('img/frontend/logos/logo-6.svg')); ?>" alt="partner">
                    </div>
                </div>	
            </div> 

            <div class="partner" data-aos="flip-down" data-aos-delay="600" data-aos-once="true" data-aos-duration="400">					
                <div class="partner-image d-flex">
                    <div>
                        <img src="<?php echo e(theme_url('img/frontend/logos/logo-3.svg')); ?>" alt="partner">
                    </div>
                </div>	
            </div> 
        </div>
    </div>

</section><?php /**PATH E:\projects\laravel\2024\Emmanuel\new_videmak\resources\views/default/frontend/clients/section.blade.php ENDPATH**/ ?>